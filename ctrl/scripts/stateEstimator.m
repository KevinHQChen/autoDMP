function [A, B, C, D, Q, R, N] = stateEstimator(Ap, Bp, Cp, Dp, Aod, Bod, Cod, Dod, Dn)
%#codegen
no = size(Cp,1);        % n_outputs
no_ = no/2;
ni = size(Bp,2);        % n_inputs
nsp = size(Ap,1);       % n_plant_states
...
D_est = [Dp Dod Dn];
Q = blkdiag(Bp(1:no_,1:ni)*Bp(1:no_,1:ni)', Bp(no_+1:no,1:ni)*Bp(no_+1:no,1:ni)', Bod*Bod');
% Q = B_est * B_est';
R = D_est * D_est';
N = B_est * D_est';
