...
// sample commands:
// For SYSID:   .\robodrop.exe --vidSrc 2 --templateSource 0 --chanSource 0 --tmThres 0.8 --ctrl 2 --dryRun 1
// For control: .\robodrop.exe --vidSrc 2 --templateSource 0 --chanSource 0 --tmThres 0.8 --ctrl 1 --dryRun 1
// (replace with REPL if we get the chance)
config::config(int argc, char* argv[]) {
    ...
    try {
        ...
        desc.add_options()
            ...
            ("vidSrcFn", po::value<std::string>(&vidSrcFn)->default_value("C:/Users/khqc/RoboDrop/rawVideo2.avi"),
                "set video source filename")
            ("templateSource", po::value<int>(&templateSource)->default_value(0),
            ...
            ("useGPU", po::value<int>(&useGPU)->default_value(0),
                "use GPU (0:No, 1:Yes)")
            ("numChannels", po::value<int>(&numChannels)->default_value(3), "set number of observable channels")
            ("dryRun", po::value<int>(&dryRun)->default_value(0), "dry Run (0:No, 1:Yes)")
        ;
...
ffstream::ffstream(std::string filename)
{
    ...
    // In Windows (and MSVC) wchar_t is preferred over char, so it's a good idea to use wide strings everywhere
    // std::wstring filename = L"ueva_log.txt";
