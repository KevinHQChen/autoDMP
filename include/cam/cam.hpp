...
// create a generic type queue class template for storing frames (typename and class are equivalent)
// you specify the type when you use it
template <typename T>
class QueueFPS : public std::queue<T> {
public:
    // constructor:
    // chrono uses the concepts of timepoints and durations
    // now() provides a single timepoint (num of ticks since epoch) from a specific clock
    // this has no meaning by itself (unless you want ticks since epoch, in which case use now().time_since_epoch().count())
    // a difference between 2 timepoints returns a duration for which the number of ticks is given by duration.count()
    QueueFPS(std::wstring fileName) : counter(0)
                                    , out(fileName)
    ...
    void push(const T& entry) {
        ...
        out << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now()-startTime).count() << ", ";
        counter += 1;
        if (counter == 1) {
