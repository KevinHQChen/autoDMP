...
private:
	...
	fgt_CHANNEL_INFO channelInfo[256];			// each idx represents one channel (numPressureChannels)
	fgt_CONTROLLER_INFO controllerInfo[256];	// each idx represents one controller (numControllers)
	...
	// unsigned int pressureIdx = 0;				// pressure channel index, 0 if first detected channel of list
	float minPressure;
	float maxPressure;
#endif
